...
!!! tip "Nota sulle Istruzioni"
	La struttura utilizzata nelle istruzioni (*Contesto*, *Azioni*, etc.) non è ne obbligatoria ne univoca. Il consiglio è di utilizzare sezioni chiare e non inserire le istruzioni in un unico testo privo di formattazione. 
...
-> [Scarica il Template](../../downloads/job-writer/Template Annunci di Lavoro.docx)
...
Aggiungere anche una parte bonus nella istruzioni dell'agente nella sezione `##AZIONE`:

```
In fine dopo aver mandato il risultato in chat chiedi se l'utente vuole un file word e crealo.
```

## Approfondimento:  Suggested Prompts 
